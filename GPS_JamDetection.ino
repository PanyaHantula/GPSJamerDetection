...
void parseGPGSV(String sentence) {
  // Example GPGSV sentence: $GPGSV,3,1,12,01,89,046,48,02,56,245,42,03,40,097,42,04,33,197,44*72
  // Split the sentence by commas
  int numFields = 0;
  String fields[20];  // Array to store split fields (max of 20 fields per sentence)
  char delimiter = ',';
  
  int startIdx = 0;
  for (int i = 0; i < sentence.length(); i++) {
    if (sentence[i] == delimiter) {
      fields[numFields++] = sentence.substring(startIdx, i);
      startIdx = i + 1;
    }
  }
  // Add the last field after the last comma
  fields[numFields++] = sentence.substring(startIdx);

  // Extracting values from the GPGSV sentence
  int totalSatellites = fields[1].toInt();  // Total number of satellites in view
  int messageCount = fields[2].toInt();     // Number of messages in this sentence
  int messageNum = fields[3].toInt();      // Message number in this sentence
  ...
  int numberOfSatellites = (numFields - 4) / 4;  // Each satellite data takes 4 fields
  for (int i = 0; i < numberOfSatellites; i++) {
    int prn = fields[4 + i * 4].toInt();         // PRN (Satellite ID)
    int elevation = fields[5 + i * 4].toInt();   // Elevation (degrees)
    int azimuth = fields[6 + i * 4].toInt();     // Azimuth (degrees)
    int snr = fields[7 + i * 4].toInt();         // Signal-to-noise ratio (dB)
